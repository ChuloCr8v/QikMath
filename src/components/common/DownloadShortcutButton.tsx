...
import { Download, X, Share2, Smartphone, Plus } from 'lucide-react';
...
export const DownloadShortcutButton = () => {
  ...
  useEffect(() => {
    // Check if device is iOS safari / chrome
  ...
  // Hide during active math game to maximize focusing area
    ...
    <>
      <div className="fixed bottom-6 right-6 z-50">
        <motion.button
          ...
          whileTap={{ scale: 0.9 }}
          className="flex items-center gap-2 px-3 sm:px-4 py-2.5 sm:py-3 bg-gradient-to-r from-amber-400 to-pink-500 text-white rounded-full font-bold text-xs sm:text-sm border-2 border-white shadow-[0_4px_12px_rgba(245,158,11,0.3)] cursor-pointer select-none"
      ...
      <AnimatePresence>
        {showInstructions && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            ...
            >
              <button
                onClick={() => { sounds.tap(); setShowInstructions(false); }}
              ...
              <p className="text-xs text-slate-500 leading-relaxed mb-4">
                Add <strong>Math Quest</strong> to your phone or tablet's home screen to play instantly anytime just like a real app!
              </p>

              <div className="space-y-3 text-left">
                {isIOS ? (
                  <div className="p-3 bg-pink-50/50 rounded-2xl border border-pink-100 text-[11px] text-slate-600 leading-relaxed">
                    <span className="font-extrabold text-pink-500 uppercase block mb-1">🍎 iOS Safari (iPhone & iPad)</span>
                    1. Tap the <strong className="text-pink-500">Share</strong> button <Share2 className="inline ml-0.5" size={12} /> in Safari.<br />
                    2. Scroll down & select <strong>'Add to Home Screen'</strong> <Plus className="inline ml-0.5 border border-slate-300 bg-white rounded" size={11} />.<br />
                  ...
                  <div className="p-3 bg-sky-50/50 rounded-2xl border border-sky-100 text-[11px] text-slate-600 leading-relaxed">
                    <span className="font-extrabold text-sky-500 uppercase block mb-1">🤖 Google Chrome (Android & PC)</span>
                    1. Tap the <strong className="text-sky-500">Menu</strong> button (three dots <span className="font-bold">⋮</span> in top-right corner).<br />
